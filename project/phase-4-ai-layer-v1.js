...
(function () {
  ...
  /* =================================================================
     ...
     ================================================================= */
  const cards = [];          // { el, running, beat, cadence, timer, kick }
  let sectionInView = false;
  let tabVisible = !document.hidden;

  function canRun() { return sectionInView && tabVisible && !RM; }
  ...
  (function emailCard() {
    ...
    let openIdx = -1, streamHandle = null, poolPtr = 0;

    function render() {
      list.innerHTML = '';
      SEED.forEach((m, idx) => list.appendChild(rowEl(m, idx)));
    }

    function rowEl(m, idx) {
      const row = ce('div', 'email' + (m.unread ? ' is-unread' : '') + (m._new ? ' slidein' : ''));
      row.dataset.idx = idx;

      const top = ce('div', 'email-row');
      const u = ce('span', 'unread'); top.appendChild(u);
      ...
      const det = ce('div', 'email-detail');
      const inner = ce('div', 'email-detail-inner');
      const sum = ce('div', 'email-summary');
      sum.innerHTML = '<i class="ti ti-sparkles"></i>';
      const sumtxt = ce('span'); sumtxt.textContent = m.summary; sum.appendChild(sumtxt);
      const draft = ce('div', 'email-draft');
      ...
      const acts = ce('div', 'draft-actions');
      ...
      inner.appendChild(sum); inner.appendChild(draft); inner.appendChild(acts);
      det.appendChild(inner); row.appendChild(det);

      row.addEventListener('click', (e) => {
        if (e.target.closest('.mini-btn')) return;
        toggle(idx, row, dtxt, m);
      });
      send.addEventListener('click', () => {
        send.innerHTML = '<i class="ti ti-check" style="font-size:13px"></i>sent';
        send.style.background = 'var(--pos)';
    ...
    function toggle(idx, row, dtxt, m) {
      const rows = list.querySelectorAll('.email');
      if (openIdx === idx) { row.classList.remove('open'); openIdx = -1; if (streamHandle) streamHandle.cancel(); return; }
      rows.forEach(r => r.classList.remove('open'));
      if (streamHandle) streamHandle.cancel();
      row.classList.add('open'); row.classList.remove('is-unread'); openIdx = idx;
      dtxt.textContent = '';
      // small beat then type the draft in
      setTimeout(() => { streamHandle = stream(dtxt, m.reply, { mode: 'word', speed: 42 }); }, RM ? 0 : 240);
    }

    // ambient: every ~7s, a new email slides in at the top, "sorting…" then tag resolves
    function beat() {
      const incoming = Object.assign({}, POOL[poolPtr % POOL.length]); poolPtr++;
      incoming._new = true;
      const wasOpen = openIdx;
      // insert at top, drop the last to keep four
      SEED.unshift(incoming); SEED.pop();
      // adjust open index (shifted down by one); close if it fell off
      if (wasOpen >= 0) { openIdx = wasOpen + 1 <= 3 ? wasOpen + 1 : -1; }
      render();
      const topRow = list.querySelector('.email');
      const chip = topRow.querySelector('.echip');
      const realChip = incoming.chip;
      chip.textContent = 'ai sorting…'; chip.classList.add('sorting');
      // re-open the previously-open row if still present
      if (openIdx >= 0) {
        const reopen = list.querySelector('.email[data-idx="' + openIdx + '"]');
        if (reopen) { reopen.classList.add('open'); const dt = reopen.querySelector('.draft-text'); dt.textContent = SEED[openIdx].reply; }
      }
    ...
    render();
    cards.push({ el: $('#card-email'), running: false, beat, cadence: 7000, timer: null });
  })();
  ...
  (function chatCard() {
    ...
    cards.push({ el: $('#card-chat'), running: false, beat, cadence: 8000, timer: null });
  })();
  ...
  (function dashCard() {
    ...
    cards.push({ el: $('#card-dash'), running: false, beat, cadence: 9000, timer: null });
  })();

  /* =================================================================
     SECTION OBSERVER — start/stop all ambient on enter/leave
     ================================================================= */
  (function () {
    const section = $('#ailayer');
    if (!section || !('IntersectionObserver' in window)) { sectionInView = true; syncCards(); return; }
